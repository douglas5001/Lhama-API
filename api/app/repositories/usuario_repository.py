...
class UsuarioRepository:
    ...
    def create(self, dados_usuario: UsuarioCreate):
        # Aqui convertemos de Pydantic DTO para SQLAlchemy Model
        db_usuario = Usuario(
            nome=dados_usuario.nome,
            email=dados_usuario.email,
            senha=dados_usuario.senha, # Em produção, gravar em hash!
            ativo=dados_usuario.ativo
        )
        self.db.add(db_usuario)
        self.db.commit()
        self.db.refresh(db_usuario) # Atualiza o objeto com os dados do banco (ex: ID gerado)
        return db_usuario
